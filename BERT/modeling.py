...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    'bert-base-uncased': "XXXX",
    'bert-large-uncased': "XXXX",
    'bert-base-cased': "XXXX",
    'bert-large-cased': "XXXX",
    'bert-base-multilingual-uncased': "XXXX",
    'bert-base-multilingual-cased': "XXXX",
    'bert-base-chinese': "XXXX",
    'bert-base-german-cased': "XXXX",
    'bert-large-uncased-whole-word-masking': "XXXX",
    'bert-large-cased-whole-word-masking': "XXXX",
    'bert-large-uncased-whole-word-masking-finetuned-squad': "XXXX",
    'bert-large-cased-whole-word-masking-finetuned-squad': "XXXX",
    'bert-base-cased-finetuned-mrpc': "XXXX",
}
...
def load_tf_weights_in_bert(model, tf_checkpoint_path):
    ...
    try:
        ...
        print("Loading a TensorFlow models in PyTorch, requires TensorFlow to be installed. Please see "
            "XXXX for installation instructions.")
        raise
...
try:
    ...
    logger.info("Better speed can be achieved with apex installed from XXXX .")
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, speaker_embeddings=False):
        ...
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0).to(device)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size).to(device)
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size).to(device)
    ...
    def forward(self, input_ids, token_type_ids=None, speaker_ids=None):
        ...
        words_embeddings = self.word_embeddings(input_ids).cuda()
        position_embeddings = self.position_embeddings(position_ids).cuda()
        token_type_embeddings = self.token_type_embeddings(token_type_ids).cuda()
